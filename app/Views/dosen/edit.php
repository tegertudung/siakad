...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card">
                <div class="card-header"><h3 class="card-title">Formulir Edit Data Dosen</h3></div>
                <form action="<?= base_url('dosen/' . $dosen['id']); ?>" method="post">
                    <?= csrf_field(); ?>
                    <input type="hidden" name="_method" value="PUT">
                    <div class="card-body">
                        <div class="form-group">
                            <label>Nama Lengkap</label>
                            <input type="text" class="form-control" name="nama_lengkap" value="<?= esc($dosen['nama_lengkap']); ?>" required>
                        </div>
                        <div class="form-group">
                            <label>NIDN</label>
                            <input type="text" class="form-control" name="nidn" value="<?= esc($dosen['nidn']); ?>" required>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="jurusan_id" id="jurusan_id" class="form-control" required>
                                ...
                                <?php foreach($jurusan as $jrs): ?>
                                    <option value="<?= $jrs['id']; ?>" <?= ($dosen['jurusan_id'] == $jrs['id']) ? 'selected' : '' ?>><?= $jrs['nama_jurusan']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="matakuliah_ids[]" id="matakuliah_ids" class="form-control" multiple>
                                <?php foreach($matakuliah as $mk): ?>
                                    <option value="<?= $mk['id']; ?>" <?= in_array($mk['id'], $id_matakuliah_diampu) ? 'selected' : '' ?>>
                                        <?= $mk['nama_mk']; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="username" value="<?= esc($dosen['username']); ?>" readonly>
                        </div>
                        <div class="form-group">
                            <label>Email</label>
                            <input type="email" class="form-control" name="email" value="<?= esc($dosen['email']); ?>" required>
                        </div>
